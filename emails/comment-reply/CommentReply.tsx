/**
 * emails/comment-reply/CommentReply.tsx
 *
 * Email template for 'comment_reply' notifications.
 */
...
export function CommentReplyEmail({
  ...
  const preview = `${actorName} replied to your comment on "${taskTitle}"`;
  return (
    <AppShell preview={preview}>
      ...
      <Text style={{ margin: "0 0 4px", fontSize: t.fontSizeBase, color: t.colorFgMuted }}>
        On <strong>{taskTitle}</strong> · {boardTitle} · {workspaceName}
      </Text>
      {commentPreview && (
        <Section
          style={{
