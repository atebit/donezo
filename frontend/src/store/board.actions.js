...
export async function updatePickerCmpsOrder(filteredBoard, newColumns) {
  try {
    const { board } = store.getState().boardModule
    board.columns = newColumns
    filteredBoard.columns = newColumns
...
export async function addTask(task, group, filteredBoard, activity) {
  try {
    ...
    // Initialize task.cells as an array with one empty string per column.
    if (!task.cells || !Array.isArray(task.cells)) {
      task.cells = board.columns.map(() => "")
    }
...
export async function addTaskOnFirstGroup(filteredBoard) {
  try {
    ...
    // Initialize new task cells to empty values.
    taskToAdd.cells = board.columns.map(() => "")
...
export function updateBoardAction(updatedBoard, changedLabelInfo) {
  return async (dispatch) => {
    try {
      ...
      dispatch({ type: UPDATE_BOARD, board, changedLabelInfo })
