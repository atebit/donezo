...
/**
 * LongTextEditor — popover textarea for the "long_text" cell type.
 *
 * Per Q11: Tiptap is NOT installed for this epic. Ships as a multi-line
 * `<textarea>` inside a Base UI `<Popover>`. The `config.richText` flag
 * is registered as `false` for v1; when Tiptap lands (epic 09), flipping
 * it to `true` will swap the editor.
 *
 * Contract:
    ...
    >
      ...
      <Popover.Portal>
        <Popover.Positioner sideOffset={0} align="start">
          <Popover.Popup
            className="z-[var(--z-popover)] bg-[color:var(--color-surface)] border border-[color:var(--color-border-strong)] rounded-[var(--radius-sm)] shadow-[var(--shadow-modal)] p-2 flex flex-col gap-2"
