...
function TagsCellInner({ value, config: _config, row: _row }: TagsCellProps) {
  ...
  return (
    <div
      ...
      className="min-w-[var(--size-cell-w)] h-[var(--size-cell-h)] border border-[color:var(--color-border-strong)] flex items-center px-2 gap-1 hover:outline hover:outline-1 hover:outline-[color:var(--color-border-strong)] overflow-hidden"
    >
      {visible.map((tag) => (
        <span
          key={tag}
